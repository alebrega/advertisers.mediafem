...
class Sitescategories extends CI_Model {
    ...
    function get_sites_in_categories($categories) {
        $query = $this->db->query("SELECT s.id, s.nombre_appnexus, s.nombre_dfp, s.id_adunit_site, s.id_site FROM sitios_categorias sc, sites s WHERE sc.id_categoria IN ($categories) AND s.id = sc.id_sitio AND s.id_site != '' GROUP BY s.id;");
        if ($query->num_rows() > 0)
            return $query->result();
        return null;
    }
