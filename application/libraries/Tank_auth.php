...
class Tank_auth {
            ...
            if (!is_null($user = $this->ci->users->$get_user_func($login))) { // login ok
                ...
                if ($hasher->CheckPassword($password, $user->password)) {  // password ok
                    ...
                    $this->ci->session->set_userdata(array(
                        ...
                        'es_agencia' => $user->agencia,
                        'tarjeta_certificada' => $user->tarjeta_certificada,
     ...
     */
    function es_agencia() {
        return $this->ci->session->userdata('es_agencia');
    }

    /**
     * Get tarjeta_certificada
     *
     * @return	string
     */
    function tarjeta_certificada() {
        return $this->ci->session->userdata('tarjeta_certificada');
    }
     ...
     * @return	array
     */
    function create_user($username, $email, $password, $email_activation, $nombre_beneficiario, $empresa, $country, $phone, $direccion, $codigo_postal, $ciudad, $provincia, $aplicar_cupon) {
        if ((strlen($username) > 0) AND ! $this->ci->users->is_username_available($username)) {
            ...
            $data = array(
                ...
                'city' => $ciudad,
                'state' => $provincia,
                'creado_desde_sitio' => '1',
                'moneda' => $moneda,
                'agencia' => 1,
                'cupon_de_promocion' => $aplicar_cupon,
            );
            ...
            if (!is_null($res = $this->ci->users->create_user($data, !$email_activation))) {
                ...
                // creo un nuevo cliente para el usuario            
                $this->ci->load->model('clientes_model');

                if ($empresa)
                    $razon_social = $empresa;
                else
                    $razon_social = $nombre_beneficiario;
                ...
                $data_cliente = array(
                    ...
                    'pais' => $country,
                    'autogestionado' => '1',
                    'creado_por' => $res['user_id'],
                    'validado_administracion' => '1',
                    'moneda' => $moneda,
                    'estado' => '1'
                );
                
                $this->ci->clientes_model->insert_cliente($data_cliente);
                $data_cliente_anunciante = array(
                    'cliente_id' => $this->ci->db->insert_id(),
                    'anunciante_id' => $res['user_id']
                );
                $this->ci->clientes_model->asignar_anunciante($data_cliente_anunciante);
