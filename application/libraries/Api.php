...
class Api {
    ...
    function crear_anunciante_appnexus($nombre, $email = null, $empresa = 'mf') {
        try {
    ...
    function get_pais_appnexus($code) {

        $pais = $this->ci->paises->get_pais_by_id($code);

        return (int) $pais->id_appnexus;
    }

    function get_pais_appnexus_api($code) {

        $request = new Request();
        $request->method = 'get';
        $request->uri = BASE_URI . '/country?code=' . $code;
        $request->token = $this->ci->token;
        $res = Caller::call($request);

        return $res->response->countries[0]->id;
    }

    function get_paises($start) {

        $request = new Request();
        $request->method = 'get';
        $request->uri = BASE_URI . '/country?start_element=' . $start . '&num_elements=100';
        $request->token = $this->ci->token;
        $res = Caller::call($request);
                ...
                if ($campania->segmentacion_id != 2) {
                    ...
                    if ($paisescampania) {
                        foreach ($paisescampania as $pais) {
                            ...
                            $tmp_pais->id = $this->get_pais_appnexus($pais->id_pais);
                    ...
                    /*
                      $modalidad = 'VIEWS';
                      if ($campania->modalidad_compra == 'cpc')
                      $modalidad = 'CLICKS'; */

                    $audiencia = $this->crear_audiencia('MF - ' . $campania->nombre . ' - VIEWS', $anunciante->id_appnexus);

                    $tmp_audiencia = new stdClass();
                    $tmp_audiencia->id = (int) $audiencia['id'];
                    $tmp_audiencia->action = "include";

                    $audiencias[] = $tmp_audiencia;

                    $audiencia = $this->crear_audiencia('MF - ' . $campania->nombre . ' - CLICKS', $anunciante->id_appnexus);
                    ...
                    if (isset($devices)) {
                        $data->profile->device_type_action = "include";
                        $data->profile->device_type_targets = $devices;
                    }
                    $request = new Request();
                    $request->method = 'post';
                    ...
                    if ($canales_campania) {
                        foreach ($canales_campania as $campania_canal) {
                            if (isset($canales_appnexus[$campania_canal->id_canal_tematico])) {
                                ...
                                unset($data, $content_categorie, $content_categories, $profile_id, $devices);
                                ...
                                if ($campania_de == 'mf') {
                                    if ($campania->segmentacion_id == 2 && $content_categories) {
                                        ...
                                        // listas de dominio
                                        unset($listas_dominios);
                                        $lista_dominio = new stdClass();
                                        $lista_dominio->id = $canales_appnexus[$campania_canal->id_canal_tematico]->id_domain_list_appnexus;
                                        $listas_dominios[] = $lista_dominio;

                                        $data->profile->domain_list_action = 'include';
                                        $data->profile->domain_list_targets = $listas_dominios;
                                    }
                                }
    ...
    function crear_campania_dfp($id_campania) {
        ...
        $data_campaign = new stdClass();
        $data_campaign->campaign = new stdClass();
        $data_campaign->campaign->id = $id_campania;
        ...
        $request = new Request();
        $request->method = 'post';
        $request->uri = 'http://adserver.adtomatik.com/new_campaign';
        $request->data = array('credentials' => $credentials, 'data' => $data_campaign);
        $request->decodeResponse = true;
        
        $res = json_decode(Caller::call_dfp($request));
        
        if(!$res->error){
            $order_id = $res->message;
            
            $update = array(
                'id_orden_dfp' => $order_id
            );
    ...
    function crear_campania_dfp2($id_campania) {
        try {
