...
abstract class AdsUser {
    ...
    public function LoadSettings($settingsIniPath, $defaultVersion, $defaultServer, $defaultLogsDir, $logsRelativePathBase) {
        ...
        // Auth settings.
        $this->authServer = $this->GetSetting($settingsIni, 'AUTH', 'AUTH_SERVER', 'https://accounts.google.com');
        // OAuth 1.0a.
        $oauthHandlerClass = $this->GetSetting($settingsIni, 'AUTH', 'OAUTH_HANDLER_CLASS');
        ...
        // OAuth2.
        $oauth2HandlerClass = $this->GetSetting($settingsIni, 'AUTH', 'OAUTH2_HANDLER_CLASS');
    ...
    private function GetAllClientLibraryUserAgentParts() {
        $allUserAgentParts[] = implode('/', $this->GetClientLibraryNameAndVersion());
     ...
     */
    public function RequestOAuthToken($callbackUrl = NULL, $server = NULL, $applicationName = NULL) {
        ...
        $this->oauthInfo = $this->GetOAuthHandler()->GetRequestToken(
                $this->oauthInfo, $scope, $this->GetAuthServer(), $callbackUrl, $applicationName);
    }
    ...
    public function GetOAuthAuthorizationUrl() {
        return $this->GetOAuthHandler()->GetAuthorizationUrl($this->oauthInfo, $this->GetAuthServer());
    }
    ...
    protected function ValidateOAuthInfo() {
        if (!array_key_exists('oauth_consumer_key', $this->oauthInfo)) {
            throw new ValidationException('oauthInfo', NULL, 'oauth_consumer_key is required and cannot be NULL.');
        }
        if (!array_key_exists('oauth_consumer_secret', $this->oauthInfo)) {
            throw new ValidationException('oauthInfo', NULL, 'oauth_consumer_secret is required and cannot be NULL.');
        }
        if (!array_key_exists('oauth_token', $this->oauthInfo)) {
            throw new ValidationException('oauthInfo', NULL, 'oauth_token is required and cannot be NULL.');
        }
        if (!array_key_exists('oauth_token_secret', $this->oauthInfo)) {
            throw new ValidationException('oauthInfo', NULL, 'oauth_token_secret is required and cannot be NULL.');
        }
    }
     ...
     */
    public function GetOAuth2AuthorizationUrl($redirectUri = NULL, $offline = NULL, array $params = null) {
        $server = isset($server) ? $server : $this->GetDefaultServer();
        $scope = $this->GetOAuth2Scope($server);
        return $this->GetOAuth2Handler()->GetAuthorizationUrl($this->oauth2Info, $scope, $redirectUri, $offline, $params);
    }
    ...
    protected function ValidateOAuth2Info() {
        if (empty($this->oauth2Info['client_id'])) {
            throw new ValidationException('oauth2Info', NULL, 'client_id is required.');
        }
        if (empty($this->oauth2Info['client_secret'])) {
            throw new ValidationException('oauth2Info', NULL, 'client_secret is required.');
        }
        if (empty($this->oauth2Info['access_token']) &&
                empty($this->oauth2Info['refresh_token'])) {
            throw new ValidationException('oauth2Info', NULL, 'access_token or refresh_token is required.');
        }
    }
