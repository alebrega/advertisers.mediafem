...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        function loadValorMinimo(){
            ...
            $.ajax({
                type: "POST",
                url: "/campania/valor_minimo/adtk",
                data: form_data,
                dataType: "json",
...
<?php } ?>
        ...
        function btn_paso_2(){
            ...
            $.ajax({
                type: "POST",
                url: "/campania/insertar_campania_appnexus_primer_paso/",
                data: form_data,
                dataType: "json",
                success: function(msg){
                    if(msg.validate){
                        ...
                        $('#seleccionar_audiencias_modal .row').html(divLoader).load('/campania/mostrar_audiencias/' + id_campana);
        ...
        $('#btn_paso_2').click(function(e){
            if($('#nuevo_anunciante').is(':visible')){
                ...
                $.ajax({
                    type: "POST",
                    url: "/campania/alta_anunciante_json/1/adt",
                    data: form_data,
                    dataType: "json",
                    success: function(msg){
                        $('#loader_anunciantes').css('display', 'none');
...
?>
        ...
        $('#btn_finalizar').click(function(e){
            ...
            if($('#chk_acepto_terminos').is(":checked")){
                ...
                $.ajax({
                    type: "POST",
                    url: "/campania/insertar_campania_appnexus_finalizar/",
                    data: form_data,
                    dataType: "json",
                    success: function(msg){
                        if(msg.validate){
...
<?php } ?>
            ...
            }else if($(this).find(':selected').val() == 'cpa'){
                $('#tipo_cantidad').html(' acciones.');
                $('#cantidad').val($('#totalClicks').val());
...
<?php } ?>
        ...
        $('a[data-reveal-id="crear_audiencia_modal"]').click(function(){
            $('#audiencia_cargada').fadeOut();
            $('#error_cargar_audiencia').fadeOut();
            $('#audiencia_name').val('');
        });

        $('#audiencia_guardar').click(function(){
            var audiencia_name = $('#audiencia_name').val();

            var form_data = {
                audiencia_name : audiencia_name,
                id_anunciante: $("#cmb_anunciantes").find(':selected').val()
            }

            $('#crear_audiencia_modal .row').fadeOut();

            $.ajax({
                type: "POST",
                url: "/campania/crear_audiencia/",
                data: form_data,
                dataType: "json",
                success: function(msg){
                    if(msg.validate){
                        var id_audiencia = msg.id_audiencia;
                        $('#audiencia_cargada').fadeIn(800);

                        $('#audiencia_name').val('');

                        $('#audiencia_cargada').fadeOut(800, function(){
                            $('.reveal-modal-bg').click();
                            $('#crear_audiencia_modal .row').fadeIn(800);
                        });

                        $('a[data-reveal-id="seleccionar_audiencias_modal"]').fadeIn(800);

                        //$('#seleccionar_audiencias_modal .row').html(divLoader).load('/campania/mostrar_audiencias/' + id_campana);

                        //$('#cmb_audiencias').html(divLoader).load('/campania/mostrar_audiencias/' + id_campana);

                        $("#cmb_audiencias").append('<option value="' + msg.id_audiencia + '">' + audiencia_name + '</option>');

                        var html = '<div class="item" id="item_' + msg.id_audiencia + '" style="display: none;"><input type="hidden" data-visible="off" data-action="include" data-name-audiencia="' + audiencia_name + '" data-id-audiencia="' + msg.id_audiencia + '" /><span class="inc-exc-toggle"><i class="include-btn active" data-referrer="item_' + msg.id_audiencia + '"></i> <i class="exclude-btn" data-referrer="item_' + msg.id_audiencia + '"></i></span> <span class="name-audiencia" title="' + audiencia_name + '"> ' + audiencia_name + ' </span> <span><i class="erase-btn" data-referrer="item_' + msg.id_audiencia + '"></i></span></div>';

                        $("div.select").append(html);
                    }else{
                        $('#error_cargar_audiencia').html(msg.error).fadeIn(800);
...
<div id="crear_audiencia_modal" class="reveal-modal medium">
    <h2>Crear nueva audiencia.</h2>

    <div class="row">
        <label style="width: auto;">Audiencia: </label>
        <input type="text" name="audiencia_name" id="audiencia_name" style="width:250px !important" />

        <input type="button" class="button_new" name="audiencia_guardar" id="audiencia_guardar" value="Crear" />
    </div>

    <div class="audiencia_cargada" id="audiencia_cargada" style="color: green; display: none;">Audiencia creada con exito.</div>

    <div class="error_cargar_audiencia" id="error_cargar_audiencia" style="color: red; display: none;"></div>

    <a class="close-reveal-modal">&#215;</a>
</div>


<div id="seleccionar_audiencias_modal" class="reveal-modal large">
    <h2>Seleccionar audiencias.</h2>
...
<div id="paso_3" style="display:none;">
    ...
    <div class="row">
        ...
        <a href="javascript:;" data-reveal-id="crear_audiencia_modal" class="button_new">
            Crear nueva audiencia
        </a>
        <a href="javascript:;" data-reveal-id="seleccionar_audiencias_modal" class="button_new"
        <?php
        if (!$audiencias) {
            echo 'style="display:none;"';
        }
