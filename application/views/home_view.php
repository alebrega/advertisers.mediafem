...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var estado_opciones_avanzadas_<?= $id_order_dfp ?> = 0; // 0: mostrandose, 1: oculto;

        $("#cmb_range_<?= $id_order_dfp ?>").change(function(){
            var range = $("#cmb_range_<?= $id_order_dfp ?>").find(':selected').val();

            if(range=='especific'){
                $("#div_fechas_especificas_<?= $id_order_dfp ?>").css("display", "block");
            }else{
                $("#div_fechas_especificas_<?= $id_order_dfp ?>").css("display", "none");
            }
        });
        ...
        //$("#tbl_reporte_" + <?= $id_order_dfp ?>).append(divLoader).load('/welcome/obtener_reporte_dinamico_status/<?= $id_order_dfp ?>', function(){
         $("#tbl_reporte_" + <?= $id_order_dfp ?>).append(divLoader).load('/welcome/obtener_reporte_dinamico_por_sitio_DFP/<?= $id_order_dfp ?>/day/REACH_LIFETIME/0/0/imps%3Bclicks%3Bctr%3B/0/0/0/0/'+seconds, function(){
        ...
        $("#btn_ejecutar_reporte_" + <?= $id_order_dfp ?>).click(function(){
            estado_opciones_avanzadas_<?= $id_order_dfp ?> = 1;
            $('#opciones_avanzadas_<?= $id_order_dfp ?>').click();
        ...
        $("#chk_sitio_<?= $id_order_dfp ?>").change(function(){
            ...
            if(thisCheck.is(':checked')){
                ...
                <?php if ($segmentacion != 3) { ?>
                    ...
                    if($("#chk_formato_<?= $id_order_dfp ?>").attr("checked"))
                    $("#chk_formato_<?= $id_order_dfp ?>").attr("checked", false);

                    if($("#chk_pais_<?= $id_order_dfp ?>").attr("checked"))
                    $("#chk_paiss_<?= $id_order_dfp ?>").attr("checked", false);
                <?php } ?>
        ...
        $('#opciones_avanzadas_<?= $id_order_dfp ?>').click(function(){
            if(estado_opciones_avanzadas_<?= $id_order_dfp ?> == 0){
                estado_opciones_avanzadas_<?= $id_order_dfp ?> = 1;
                $('#panel_opciones_avanzadas_<?= $id_order_dfp ?>').fadeIn(800);
                $(this).html('[-] Opciones avanzadas');
            }else{
                estado_opciones_avanzadas_<?= $id_order_dfp ?> = 0;
                $('#panel_opciones_avanzadas_<?= $id_order_dfp ?>').fadeOut(400);
                $(this).html('[+] Opciones avanzadas');
            }
        });
    });
...
<body class="ex_highlight_row">
    ...
    <div style="overflow:hidden;">
        <div class="floatLeft" style="padding-right: 15px; border-right: 1px solid #eee; margin-right: 15px;">
            ...
            <select id="cmb_range_<?= $id_order_dfp ?>" name="cmb_range_<?= $id_order_dfp ?>">
                ...
                <option value="LAST_WEEK">Ultimos 7 dias</option>
                <option value="MONTH_TO_DATE">este mes: <?= getMesEsp(date('m')) ?></option>
                <option value="LAST_MONTH">el mes pasado: <?= getMesEsp(date('m') - 1) ?></option>
                <option value="especific">Fechas Especificas</option>
                <option selected="selected" value="REACH_LIFETIME">Siempre</option>
            </select>
            ...
            <div id="div_fechas_especificas_<?= $id_order_dfp ?>" style="padding:20px 0 20px 47px; display: none;">
        ...
        <a href="javascript:;" id="opciones_avanzadas_<?= $id_order_dfp ?>" style="padding-top:2px; display:inline-block;">[+] Opciones avanzadas</a>
    </div>

    <hr />

    <div id="panel_opciones_avanzadas_<?= $id_order_dfp ?>" style="display: none;">

        <table class="tabla" cellspacing="30" width="70%">
            <tr class="encabezado">
            ...
            <tr>
                ...
                <td style="width: 15%;border-right: 6px solid #FFF;"><input class="chk_intervalo_<?= $id_order_dfp ?>" type="checkbox" value="day" checked="checked" name="chk_intervalo_<?= $id_order_dfp ?>[]" id="chk_dia_<?= $id_order_dfp ?>" />D&iacute;a</td>
            </tr>
            ...
            <tr>
                ...
                <td id="td_chk_convs" style="border-right: 6px solid #FFF">
                    <!--<input type="checkbox" value="total_convs" name="chk_columnas_<?= $id_order_dfp ?>[]" id="chk_convs_<?= $id_order_dfp ?>" />Acciones-->
                    
                    <?php if (!$es_agencia) { ?>
                        <input type="checkbox" value="total_revenue" name="chk_columnas_<?= $id_order_dfp ?>[]" id="chk_costo_<?= $id_order_dfp ?>"/>Costo
                     <?php } else { ?>
                        &nbsp;
                    <?php } ?>
                </td>
                ...
                <td>
                    <!--<input class="chk_grupos_<?= $id_order_dfp ?>" type="checkbox" value="creative_name" name="chk_grupos_<?= $id_order_dfp ?>[]" id="chk_creatividad_<?= $id_order_dfp ?>"/>Creatividad-->
                </td>
                <td style="width: 15%;border-right: 6px solid #FFF;">
                <!--
                    <input class="chk_intervalo_<?= $id_order_dfp ?>" type="checkbox" value="month" name="chk_intervalo_<?= $id_order_dfp ?>[]" id="chk_mes_<?= $id_order_dfp ?>" />Mes
                -->
                ...
                <td style="width: 15%;border-right: 6px solid #FFF;"></td>
            </tr>

            <tr>
                    <td style="border-left: 6px solid #FFF;">&nbsp;</td>
                <td style="border-right: 6px solid #FFF;">
                    <?php if ($tipo_campania == 'pre_roll') { ?>
                        <input type="checkbox" value="total_views" name="chk_columnas_<?= $id_order_dfp ?>[]" id="chk_vistas_<?= $id_order_dfp ?>" checked="checked" />Vistas
                    <?php } else { ?>
                        &nbsp;
                    <?php } ?>
                </td>
    ...
    <div style="margin-top: -2px;float: left; width: 100%; clear: both;" id="tbl_reporte_<?= $id_order_dfp ?>"></div>
