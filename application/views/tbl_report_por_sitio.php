...
if (isset($arr_data)) {
    foreach ($arr_data as $c => $v) {
        ...
        $fields[$posicion_site_id] = trim((string) $fields[$posicion_site_id]);
        ...
        if (in_array($fields[$posicion_site_id], $sitios_ocultos)) {
            ...
            $arr_sitios[$fields[$posicion_site_id]] = array('name' => $fields[$posicion_site_name], 'imps' => $imps_reales, 'clicks' => $clicks_reales, 'convs' => $convs_reales);
        } else {
            if (strlen($posicion_imps)) {
...
/*
  echo "Total imps reales: " . $total_imps_reales . "<br/>";
  echo "Total imps ocultas: " . $total_imps_ocultas . "<br/>";
 */
$sitios = null;
...
<table class="display" id="tbl_report_<?= $lineItem ?>">
    ...
    <tbody>
        <?php
        if (isset($arr_sitios)) {
            foreach ($arr_sitios as $c => $v) {
                $total_reg++;

                echo '<tr>';

                for ($i = 0; $i < count($arr_columnas); $i++) {
                    switch ($arr_columnas[$i]) {
                        case 'site_name':
                            echo '<td>' . $arr_sitios[$c]['name'] . '</td>';
                            break;

                        case 'imps':
                            $imps = $v['imps'];
                            if (isset($sitios[$c]))
                                $campo = $imps + round($sitios[$c]['imps_ocultas']);

                            if ($usuario->notacion == 0) {
                                $dato = number_format($campo, 0, '.', ',');
                        ...
                        case 'clicks':
                            ...
                            if (isset($sitios[$c]))
                                $campo = $clicks + round($sitios[$c]['clicks_ocultos']);
                        ...
                        case 'ctr':
                            ...
                            if (isset($sitios[$c])) {
                                $imps = $imps + round($sitios[$c]['imps_ocultas']);
                                $clicks = $clicks + round($sitios[$c]['clicks_ocultos']);
                            }

                            $campo = ($clicks / $imps) * 100;

                            if ($usuario->notacion == 0) {
                                $dato = number_format($campo, 2, '.', ',');
                            } else if ($usuario->notacion == 1) {
                                $dato = number_format($campo, 2, ',', '.');
                            }
                            echo '<td>' . $dato . '%</td>';
                        ...
                        case 'total_convs':
                            $convs = $sitios[$c]['convs_ocultas'];

                            if (isset($sitios[$c]))
                                $campo = $convs_ocultas + round($sitios[$c]['convs_ocultas']);
