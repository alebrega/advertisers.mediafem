...
class Auth extends CI_Controller {
    ...
    function activate() {
        ...
        /* Consulto los datos del usuario y si cupon_de_promocion es 1 genero el cupon */
        $anunciante = $this->anunciantes->get_anunciante_by_id($user_id);
            ...
            if ($cupon->consumido == 0) {
                ...
                if ($this->cupones_model->update($cupon->id, $update)) {
                    ...
                    $data_insert2 = array(
                        ...
                        'descripcion' => 'Carga de cupón de promocion (Cod. ' . $cupon->codigo . ').',
                        'tipo_de_pago' => 1
                    );
    ...
    function _send_email($type, $email, &$data) {
        ...
        if ($type == 'activate') {
            $this->email->subject('Bienvenido a MediaFem para Anunciantes');
        } else {
            $this->email->subject('¿Olvidaste tu contraseña en MediaFem para Anunciantes?');
        }
