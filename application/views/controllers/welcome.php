...
class Welcome extends CI_Controller {
    ...
    function obtener_reporte_dinamico_por_sitio_DFP($orden_id, $intervalo, $rango, $fecha_desde, $fecha_hasta, $columnas, $dimensiones, $filtros_li, $filtros_cr, $filtros_paises, $seconds) {
        ...
        do {
            $data['reporte'] = $this->api->obtenerReporteDinamico_DFP($orden_id, $intervalo, $rango, $fecha_desde, $fecha_hasta, $columnas, $dimensiones, $filtros);
        } while ($data['reporte'] == false);

        if ($data['reporte'] != 'ERROR') {
            foreach ($data['reporte'][2] as $value) {
                $arr_columnas[] = $value;
            }

            foreach ($data['reporte'][3] as $value) {
                $arr_columnas[] = $value;
            }

            $data['totales'] = $data['reporte'][4];

            $arr_columnas[] = $intervalo;
            ...
            $data['columnas'] = $arr_columnas_ordenado;
            ...
            foreach ($data['reporte'][0] as $row) {
                $graph_categories .= '"' . $row["fecha"] . '" ,';
                $graph_imps .= $row["impresiones"] . ' ,';

                if (isset($row["vistas"]))
                    $graph_views .= $row["vistas"] . ' ,';

                $graph_clicks .= $row["clicks"] . ' ,';
            }
        ...
        $this->load->view('tbl_report_por_sitio_dfp', $data);
    }
    ...
    function obtener_reporte_dinamico_por_sitio($anunciante_id, $rango, $filtros_li, $grupos, $columnas, $fecha_desde, $fecha_hasta, $filtros_paises, $filtros_cr, $filtros_sizes, $interval, $timezone, $orden, $direccion, $seconds) {
        ...
        /*
          ...
          echo $row_site['id']." - ".$row_site['site_name']."<br/>";
          }
         */
        foreach ($arr_sites as $row_site) {
    ...
    function create_pdf() {
        ...
        $html .= '<div>
                    ...
                    <tr>
                        <td style="color:#943634;"><b>Anunciante: <u>' . $anunciante . '</u></b></td>
                    </tr>
                    <tr>
                        <td>&nbsp;</td>
                    </tr>
                    <tr>
                        <td style="color:#943634;"><b>Campa&ntilde;a: <u>' . $orden . '</u></b></td>
                    </tr>
                    ';
