...
class Campania extends CI_Controller {
    ...
    function test($campania) {
        ...
        //new_var_dump($this->campanias->get_audiencias_by_campania(1475));
    }
    ...
    function index() {
        ...
        if ($this->creado_desde_sitio) { // si es alguien creado desde el sitio
            if ((int) $this->limite_de_compra > 0) { // y si tiene cred a favor
                $data['mostrar_crear_campania'] = TRUE; // mostrame la campania
            } else { // si no tiene cred a favor
                $data['mostrar_poner_tarjera'] = TRUE;
            }
        } else {
            if ((int) $this->limite_de_compra > 0)
                $data['mostrar_crear_campania'] = TRUE;
        }

        if ($this->user_data->no_gastar_saldo) {
            $data['mostrar_crear_campania'] = TRUE;
            $data['mostrar_poner_tarjera'] = FALSE;
        }

        $data['tiene_campanias'] = $this->tiene_campanias;
//$data['tiene_campanias'] = TRUE;
...
// si el usuario modifico la inversion de la campania.
        ...
        /*
          // averiguo lo consumido al momento de solicitar el cambio de inversion.
          $consumido = $this->api->reporte_by_orden_DFP($campania->id_orden_dfp);
          if ($consumido >= $inversion_neta) {
          echo json_encode(array('validate' => FALSE, 'error' => 'La inversi&oacute;n neta ingresada no debe ser menor o igual a lo consumido.'));
          die();
          }
         */
...
// Envio el correo al usuario para informar la modificacion de la campania
        ...
        $titulo = 'Campa&ntilde;a modificada: ' . $nombre;
...
// si se cargo todo correcto en DFP entonces cobro la campa&ntilde;a al anunciante
                    ...
                    $dias_campania = $this->_dateDiff($camp->fecha_inicio, $camp->fecha_fin);
                    $importe_a_cobrar = ($camp->inversion_neta * $dias_campania);
...
// averiguo el idiaoma actual del usuario perteneciente al sitio
            $usuario = $this->users->get_user_by_id($user_id, 1);
            if ($usuario) {
...
// averiguo la cotizacion del dolar en base a la moneda seleccionada al crear la campana.
            ...
            if ($ultimo_pago) {
                ...
                $data_ingresos = array("concept" => "Ingresos " . $descripcion, "credit" => $importe, "pago" => "0", "user_id" => $user_id,
                    "balance" => $balance, "fecha" => date('Y-m-d H:i:s'));
                ...
                if ($sites) {
                    foreach ($sites as $site) {
                        ...
                        if ($sitio) {
                            ...
                            if ($sitios['nombre'] == '' || $sitios['nombre'] == null)
...
// contabilizo las impresiones de los sitios seleccionados y compruebo
        if ($segmentacion == 3) {
            ...
            $inventario = $this->inventario_anunciantes->get_inventario_by_sites($in_sitios);
...
// creo el comentario
        $this->insert_historial($id_campania, 1, 'Campa&ntilde;a creada ' . $nombre_campania . '.');
    ...
    function crear_ticket_zendesk($id_campania, $tipo = 'nuevo') {
        ...
        if (!$campania) {
            ...
            if ($tipo == 'nuevo') {
                ...
                $titulo = 'Nueva campa&ntilde;a creada: ' . $campania->nombre . ' ** ' . $traficker->email;
...
//return $this->api->obtener_canales_tematicos();
    ...
    function get_publishers_y_sitios_json() {
         ...
         *
         */
        $sites = $this->sites->get_all_sites('nombre_dfp', 'asc');

        if ($sites) {
            foreach ($sites as $sitio) {
                if ($sitio->nombre_dfp != '' && $sitio->nombre_dfp != null) {
                    $sitios[] = array(
                        'id_sitio' => $sitio->id,
                        'url' => $sitio->nombre_dfp
                    );
...
//echo json_encode(array('validate' => TRUE));
    ...
    function alta_anunciante_json($adserver = 0) {
        $nombre = trim($this->input->post('nombre'));
...
// la campa&ntilde;a esta activada?
    ...
    function _send_email($titulo, $contenido, $copia = FALSE) {
        ...
        $mail->Host = "pop.emailsrvr.com";      // establecemos GMail como nuestro servidor SMTP
        ...
        $mail->CharSet = 'UTF-8';
//$mail->AddAddress('media@mediafem.com');
