...
header("Content-Disposition: attachment; filename=\"MediaFem - Reporte por Sitios.xls\"");
...
foreach ($arr_tabla as $row) {
    ...
    /*
    $cats = str_replace('Ã¡', 'á', $cats);
    $cats = str_replace('Ã©', 'é', $cats);
    $cats = str_replace('Ã­', 'í', $cats);
    $cats = str_replace('Ã³', 'ó', $cats);
    $cats = str_replace('Ã­', 'ú', $cats);
    */
    ...
    if ($site_name != "Totales:") {
        ...
        $arr_sites[] = array('site_name' => $site_name, 'imps' => $imps, 'cats' => $cats);
    }
}
